...
def validate(root: Path) -> dict:
    ...
    for key in ('TORCH_VERSION', 'TORCHVISION_VERSION', 'CUDA_VARIANT', 'PYTHON_MIN'):
        if key not in declarations:
            raise ReleaseError(f'Installer must declare {key}')
    for key in ('TORCH_VERSION', 'CUDA_VARIANT', 'PYTHON_MIN'):
        if declarations[key] not in meta['body']:
    ...
    if command not in script:
        raise ReleaseError('Installer must contain the declared Torch command')
...
class GitHub:
    ...
    def get_release(self, tag: str) -> dict | None:
        # GitHub's by-tag endpoint excludes drafts, even for authenticated owners.
        published = self.api(f'releases/tags/{tag}', missing=True)
        if published is not None:
            return published
        output = self.run('gh', 'api', f'repos/{self.repo}/releases', '--paginate',
    ...
    def publish(self, meta: dict, commit: str) -> str:
        ...
        if target is not None and target != commit:
            ...
            # Permit automation-only fixes to finish an identical interrupted draft.
            # Its installer assets, metadata and original tag must remain unchanged.
            self.snapshot(target, meta['assets'], metadata=True)
        ...
        uploaded = self.get_release(tag)
        if uploaded is None:
            raise ReleaseError('Uploaded draft could not be found; retry without moving its tag')
